...
It also syncs the installed `openai-bundled` marketplace from the current Codex package into a stable local root, overlays a local `computer-use@openai-bundled` compatibility plugin, writes that marketplace into config, and registers `browser@openai-bundled` through the user-accessible Codex CLI. Pass `-InstallAllBundledPlugins` when the user asks to restore every non-remote bundled feature; the wrapper then registers and verifies every complete descriptor currently offered by the stable marketplace, including version-dependent plugins such as `deep-research` and `visualize`, instead of relying on a fixed plugin list. When `.codex\.tmp` is a junction, the stable root is created beside the junction target so an existing non-system-drive layout stays off C:; otherwise it uses `.codex\marketplaces\openai-bundled-local`. It also repairs stable `browser` / `chrome` plugin cache copies so their `latest` junctions do not point at the mutable `.tmp` marketplace mirror, and enables `CODEX_ELECTRON_ENABLE_WINDOWS_COMPUTER_USE=1` for the current user so the Desktop app can expose Windows Computer Use after restart.
...
- `-InstallAllBundledPlugins`: during a full repair, register and verify every complete plugin descriptor in the stable `openai-bundled` marketplace. Use this when the user asks for all bundled features; targeted Computer Use repairs retain the narrower default.
...
- When `-InstallAllBundledPlugins` is requested, every complete descriptor in the stable `openai-bundled` marketplace is `installed, enabled`; on Desktop `26.727.6591.0` this additionally includes `deep-research@openai-bundled` and `visualize@openai-bundled`.
